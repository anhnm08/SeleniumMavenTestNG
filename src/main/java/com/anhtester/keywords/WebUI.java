...
//    }
    ...
    //Tạm thời dùng để code buoi 16 không bị lỗi
...
//        WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(TIMEOUT));
//        wait.until(ExpectedConditions.visibilityOfElementLocated(by));
        // waitForElementVisible(by); //Cmt ở Buoi46
...
//        wait.until(ExpectedConditions.visibilityOfElementLocated(by));
    ...
    //Chức năng của 2 hàm dưới đây giống 3 hàm trên, chỉ là 1 cách làm khác
    ...
    //2 hàm dưới giống hệt nhau, chỉ khác tên để nhớ tên nào thì dùng hàm đó
    ...
    // Hàm Highlight gọi ở các hàm action: clickElement,

    //_________________________________________________________________________________________

    //Wait for Element_basic_buoi45
    ...
    /*
    Khi import thư viện testNG trong pom.xml: có 1 dòng code <scope>test<scope>
    Lưu ý cần bỏ dòng code này, do nếu có nó thì assert là của testNG chỉ áp dụng cho package test,
    trong khi WebUI lại nằm ở package main
     */
...
//_________________________________________________________________________________________
    //Wait for Element_advanced_buoi46

    //Cmt do trùng 2 hàm trên đã paste ở buoi45
//    public static void waitForElementVisible(By by) {
//        try {
//            WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(TIMEOUT), Duration.ofMillis(500));
//            wait.until(ExpectedConditions.visibilityOfElementLocated(by));
...
//    }
     ...
     */
    public static void waitForPageLoaded() {        //Gọi hàm này ở vị trí nào sau khi chuyển trang, hoặc trước khi thao tác ở trang mới
...
// ____________________
    ...
    public static boolean verifyContains(String actual, String expected) { // Hàm này nhằm lấy ra giá trị đúng/ sai
        waitForPageLoaded();
    ...
    public static void assertContains(String actual, String expected, String message) { // Hàm này quyết định testcase đó pas/fail
        waitForPageLoaded();
        System.out.println("Assert contains: " + actual + " and " + expected);
        boolean check = actual.contains(expected);
        Assert.assertTrue(check, message); //Không return kết quả, mà quyết định luôn đúng sai, thường dùng hơn
    }

    public static void assertContains(String actual, String expected) { // Ver bỏ message
        waitForPageLoaded();
        System.out.println("Assert contains: " + actual + " and " + expected);
        boolean check = actual.contains(expected);
